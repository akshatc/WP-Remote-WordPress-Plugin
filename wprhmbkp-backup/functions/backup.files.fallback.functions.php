 ...
 * @param string $path
 */
function wpr_hmbkp_archive_files_fallback( $path ) {
	...
	if ( ( defined( 'WPRP_HMBKP_DATABASE_ONLY' ) && !WPRP_HMBKP_DATABASE_ONLY ) || !defined( 'WPRP_HMBKP_DATABASE_ONLY' ) )
		$archive->create( ABSPATH, PCLZIP_OPT_REMOVE_PATH, ABSPATH, PCLZIP_CB_PRE_ADD, 'wpr_hmbkp_pclzip_exclude' );

	// Only zip up the database
	if ( defined( 'WPRP_HMBKP_DATABASE_ONLY' ) && WPRP_HMBKP_DATABASE_ONLY )
		$archive->create( wpr_hmbkp_path() . '/database_' . DB_NAME . '.sql', PCLZIP_OPT_REMOVE_PATH, wpr_hmbkp_path() );
 ...
 * @return bool
 */
function wpr_hmbkp_pclzip_exclude( $event, &$file ) {
