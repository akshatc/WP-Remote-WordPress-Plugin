...
//            throw new Exception( sprintf( __( 'Invalid root path %s must be a valid directory path', 'wpremote' ), '<code>' . $path . '</code>' ) );
    ...
    public function do_unzip($archive, $destination)
    {
        ...
        if ( $this->get_zip_command_path() ) {
            return $this->unzip_file($archive, $destination);
        }
    ...
    public function unzip_file($archive, $destination) {
        return shell_exec( escapeshellcmd( $this->get_zip_command_path() ) . ' ' . escapeshellarg( $archive ) . ' -d ' . $destination . ' 2>&1' );
    }
