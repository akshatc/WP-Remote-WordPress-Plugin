 ...
 */
class WPRP_Backup extends WPRP_HM_Backup {
    ...
    /**
     * Return the current instance of WPRP_BackupRepo
     *
     * @static
     * @access public
     */
    ...
    public function __construct() {
        ...
        } elseif ( isset( $_GET['backup_excludes'] ) ) {
        ...
        switch ( $action ) :
            ...
            case 'hmbkp_archive_started' :
                if ( $this->is_using_file_manifest() ) {
                    $status = sprintf(__('%d files remaining to archive %s', 'wpremote'),
                        $this->file_manifest_remaining, '(<code>' . $this->get_archive_method() . '</code>)');
                } else {
                    $status = sprintf(__('Creating zip archive %s', 'wpremote'),
                        '(<code>' . $this->get_archive_method() . '</code>)');
                }
    ...
    private function key() {
        if ( ! empty( $this->key ) ) return $this->key;
    ...
    private function get_status() {
        ...
        return file_get_contents( $this->get_schedule_running_path() );
    }
    ...
    public function setPluginDetails( $plugin )
    {
        ...
        $archive = 'plugin-tmp-backup-' . $plugin_path . '.zip';
